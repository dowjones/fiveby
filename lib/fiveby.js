  ...
  Object.keys(this.config.browsers).forEach(function (elem) {
    ...
    lastPromise.then(function() {
      ...
      var cap = self.config.browsers[elem],
        logToHar = (self.config.harFileName && capabilities.caps_.browserName === 'phantomjs') ? true : false;
      ...
      self.registerHook('fiveby cleanup', describe, "afterAll", function () {
        ...
        if (logToHar) {
          driver.manage().logs().get('har').then(function (logobject) {
            self.writeHarFile(self.config.harFileName, logobject[0].message);
          });
        }
...
fiveby.prototype.writeHarFile = function (filename, filecontents) {
  var fs = require('fs'), directoryPath = "./harfiles";

  if(!fs.existsSync(directoryPath)){
    fs.mkdirSync(directoryPath, 0766, function(err){
      if(err){
        console.error(err);
        response.send("ERROR! Can't make the directory for har files! \n");    // echo the result back
      }
    });
  }
  fs.writeFile(directoryPath + "/" + filename, filecontents, function( err ) {
    if(err) {
      console.error(err);
    } else {
