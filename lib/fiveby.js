...
fiveby.prototype.localServer = function () {
  if (!global.fivebyConfig.hubUrl && !this.config.hubUrl) {
    console.info("No server defined, spinning one up ...");
    var helper = new Helper();
    helper.isJava()
      .then(helper.isJar)
      .then(helper.download)
      .then(this.runSeleniumServer);
  } else if (!global.testPromise) {
...
fiveby.prototype.runSeleniumServer = function () {
  ...
  server.start();
  global.testPromise = global.fivebyConfig.hubUrl = this.config.hubUrl = server.address();
};
